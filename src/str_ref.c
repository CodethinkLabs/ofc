...
ofc_str_ref_list_t* ofc_str_ref_list_create(void)
{
	ofc_str_ref_list_t* list
		= (ofc_str_ref_list_t*)malloc(
			sizeof(ofc_str_ref_list_t));
	if (!list) return NULL;
	...
	ofc_str_ref_t ref)
{
	...
	ofc_str_ref_t* nref = (ofc_str_ref_t*)realloc(list->ref,
		(sizeof(ofc_str_ref_t) * (list->count + 1)));
	if (!nref) return false;
	list->ref = nref;

	list->ref[list->count++] = ref;
	return true;
}
